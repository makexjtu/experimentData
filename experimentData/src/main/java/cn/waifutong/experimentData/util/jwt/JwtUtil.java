...
public class JwtUtil {
    ...
    public static String createJWT(String userId, String userType) {
        ...
        JwtBuilder builder = Jwts.builder()          // 这里其实就是new一个JwtBuilder，设置jwt的body
                ...
                //.setIssuer(custom.getRealName())   // issuer：jwt签发人
     ...
     * @return
     */
    public static Boolean isVerify(String token, String userId, String userType) {
        //得到DefaultJwtParser
        Claims claims = parseJWT(token);

        if (claims.get("userId").equals(userId) && claims.get("userType").equals(userType)) {
            return true;
        }
     ...
     * @return
     */
    public static Boolean isVerify(String token, String userType) {
        //得到DefaultJwtParser
        Claims claims = parseJWT(token);

        if (claims.get("userType").equals(userType)) {
            return true;
        }
    ...
    /**
     * 从前台的requestHeader里直接取出用户id
     * 前台在请求是应将Header中的Authorization参数设置成token的值
     * @param request
     * @return
     */
